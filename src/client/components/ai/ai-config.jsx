...
const defaultRoles = [
  ...
  {
    value: 'ç»ˆç«¯ä¸“å®¶,æä¾›ä¸åŒç³»ç»Ÿä¸‹å‘½ä»¤,ç®€è¦è§£é‡Šç”¨æ³•,ç”¨markdownæ ¼å¼'
  }
]
...
export default function AIConfigForm ({ initialValues, onSubmit, showAIConfig }) {
  ...
  function renderApiKeyLabel () {
    if (baseURLAI === 'https://api.atlascloud.ai/v1') {
      return <span className='bold'>API Key (<Link to='https://www.atlascloud.ai/?utm_source=sarmaterm_app&utm_medium=link&utm_campaign=sarmaterm'>get API key from atlascloud</Link>)</span>
    }
    if (baseURLAI === 'https://ai.sarmaterm.org/api/ai') {
      return <span className='bold'>API Key (<Link to='https://ai.sarmaterm.org?utm=sarmaterm'>get API key from ai.sarmaterm.org(free)</Link>)</span>
    }
    return 'API Key'
  }
