    ...
    once (handler) {
      ...
      queueMicrotask(() => {
        if (opts?.mode === 'confirm') {
          ...
          // Fail loudly â€“ agent login should not need interactive prompts
          handler({ results: [] }) // empty results â†’ reject('User cancel')
        }
      })
      pendingHandler = null
    },
...
// â”€â”€â”€ fixture management â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
describe('SSH agent auth', () => {
  ...
  test(
    ...
    async (t) => {
      ...
      let agent
      try {
        agent = startAgent() // start without a key first â€“ we load it below
      } catch (err) {
        if (err.code === 'ENOENT') {
  ...
  test(
    ...
    async (t) => {
      ...
      // Expose the agent socket via the environment variable only â€“ do NOT
      ...
      try {
        term = await session({
          ...
          port: server.port,
          username: USERNAME,
          // No sshAgent option â€“ should discover SSH_AUTH_SOCK automatically
          useSshAgent: true,
          enableSsh: false,
          readyTimeout: 10000,
          debug: true
