...
// Default for backward compat â€” instantiated in beforeEach
let simulateWatcher
...
describe('SQLite DB concurrency (DatabaseSync)', function () {
  ...
  test('should handle rapid fire: running guard prevents duplicate work', async function () {
    ...
    // Concurrent calls are skipped â€” this prevents the cascading duplicate work
  ...
  test('should handle bulk insert of 500 items - running guard allows one cycle', async function () {
    // 5 concurrent watcher calls for the same dbName â€” only one runs
    const promises = []
    for (let w = 0; w < 5; w++) {
      const items = Array.from({ length: 100 }, (_, i) => ({
